...
<div bind:this={wrapperEl} class="sticky-scroll-wrapper" style={`height: calc(100vh * ${tabs.length});`}>
    <Section lines="XXXXX" sectionLayoutClass="sticky-layout" borderBottom={true}>
        <Column span={12} class="py-8">
            <p class="subheading text-accent text-center">sit amer constroctoru</p>
            <h2 class="text-center">LOREM ISPUM DOLOR</h2>
        </Column>

        <Column span={6} class="py-2">
            {#if activeTab.id === "tab-1"}
                <h2>Tab 1 content mhmmhm</h2>
                <p>Lorem ipsum, dolor sit amet consectetur adipisicing elit. Quod iure suscipit eveniet corporis minus quaerat aperiam magnam dolorum, ad quisquam!</p>
            {:else if activeTab.id === "tab-2"}
                <h2>Tab 2 content mhmmhm</h2>
                <p>Lorem ipsum, dolor sit amet consectetur adipisicing elit. Quod iure suscipit eveniet corporis minus quaerat aperiam magnam dolorum, ad quisquam!</p>
            {:else if activeTab.id === "tab-3"}
                <h2>Tab 3 content mhmmhm</h2>
                <p>Lorem ipsum, dolor sit amet consectetur adipisicing elit. Quod iure suscipit eveniet corporis minus quaerat aperiam magnam dolorum, ad quisquam!</p>
            {:else if activeTab.id === "tab-4"}
                <h2>Tab 4 content mhmmhm</h2>
                <p>Lorem ipsum, dolor sit amet consectetur adipisicing elit. Quod iure suscipit eveniet corporis minus quaerat aperiam magnam dolorum, ad quisquam!</p>
            {/if}
        </Column>

        <Column span={6}>
            <!-- this container has to have a flexible height, otherwise this section doesn't function properly on a small screen -->
            <!-- we could also use media queries based on height -->
            <StripedContainer>
                <h2 class="py-8 px-2">lorem ipsum</h2>
            </StripedContainer>
        </Column>
